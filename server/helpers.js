const { CARD_TYPES, TOKEN_VALUES, BONUS_TOKENS, MARKET_SIZE } = require('./deck.js');

const HAND_LIMIT = 7;
const PRECIOUS_GOODS = new Set([
  CARD_TYPES.DIAMOND,
  CARD_TYPES.GOLD,
  CARD_TYPES.SILVER
]);
...
function isRoundOver(deck, tokenStacks) {
  return (
    deck.length === 0 ||
...
function calculateCamelBonus(player1Camels, player2Camels) {
...
function sellGoods(cards, tokenStacks, bonusStacks) {
  ...
  if (count < minRequired) {
    return {
      error: `Butuh minimal ${minRequired} ${getCardLabel(
        cardType
      )} untuk menjual`
    };
  }
  ...
  const { bonusTokens, updatedBonusStacks } = pullBonusTokens(
    count,
    bonusStacks
  );
...
function takeGoodsCard(
  ...
  const goodsCount = playerHand.filter(
    item => item.type !== CARD_TYPES.CAMEL
  ).length;
  if (goodsCount >= handLimit) {
    return {
      error: `Jumlah barang di tangan sudah mencapai batas (${handLimit})`
    };
  }
  if (goodsCount + 1 > handLimit) {
    return {
      error: `Mengambil kartu ini akan melewati batas ${handLimit} barang di tangan`
    };
  }
...
function takeCamels(market, deck, playerCamelHerd) {
...
function initializeTokenStacks() {
  const stacks = {};
...
function initializeBonusTokens() {
  return {
...
function getCardAsset(cardType) {
  return `/cards/${cardType}.png`;
}

function pullBonusTokens(
  count,
  bonusStacks = { three: [], four: [], five: [] }
) {
  ...
  if (count === 3 && updatedBonusStacks.three.length) {
    bonusTokens = [
      { type: 'three', value: updatedBonusStacks.three.shift() }
    ];
